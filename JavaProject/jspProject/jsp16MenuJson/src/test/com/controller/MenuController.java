...
public class MenuController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (sPath.equals("/index.do")) {
			...
			vo.setNum(6);
			MenuVO tvo = mService.selectOne(vo);
			...
			response.getWriter().append("{\"menu\":\""+mvo.getMenu()+"\", \"price\":"+mvo.getPrice()+","
										+ "\"store\":\""+mvo.getStore()+"\", \"kcal\":"+mvo.getKcal()+"");
			response.getWriter().append("]");
			*/
			...
			for (int i = 0; i < list.size(); i++) {
				response.getWriter()
						.append("{\"menu\":\"" + list.get(i).getMenu() + "\"," + " \"price\":" + list.get(i).getPrice()
								+ "," + " \"store\":\"" + list.get(i).getStore() + "\",\"kcal\":"
								+ list.get(i).getKcal() + "}");
				if (i != list.size() - 1)
					response.getWriter().append(",");
			}
