					...
					<table>
						...
						<tbody>
							<c:forEach items="${list}" var="vo">
								<tr>
									<td>${vo.board_num}</td>
									<td><a href="BoardContent.do?board_num=${vo.board_num}">${vo.content}</a></td>
									<td>${vo.board_date}</td>
									<td>${vo.id}</td>
									<td><a href="DeleteBoard.do?board_num=${vo.board_num}">삭제</a></td>
								</tr>
							</c:forEach>
						</tbody>
						<tfoot>
							<tr>
								<td colspan="6"><c:forEach var="i" begin="1"
										end="${totalPage}" step="1" varStatus="status">
										<a href="BoardList.do?cpage=${i}">${status.count}&nbsp;&nbsp;&nbsp;</a>
									</c:forEach></td>
							</tr>
						</tfoot>
					</table>
