...
public class MenuController {
	...
	public String insertFileOK(@ModelAttribute("tvo") TestVO vo, HttpServletRequest request)
		...
		logger.info("method insertOk! getName = {}", vo.getName());
		logger.info("method insertOk! getTel = {}", vo.getTel());
		String imgName = "";
		...
		if (null != multipartFile) {
			...
			/*절대경로와는 다른 realPath. 돌고있는 서버의 경로이다. 여기에 넣어야만 바로 이미지가 들어가서 보이게 된다.
			 * 로컬에 있는 폴더에 write하면 새로고침을 하지 않으면 인식하지 않는다.
			 * realpath에는 파일 아웃풋과 썸네일 이미지(경량화한) 두 개가 생긴다. 웹에 뜨는 건 썸네일을 가져온 것. */
